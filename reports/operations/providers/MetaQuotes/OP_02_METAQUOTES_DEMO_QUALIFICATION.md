...
3. Run `python -m tools.st_c5_3_connection_check`.
4. Record terminal build, server, account type, timestamp, and symbol metadata.
5. Run `python -m tools.st_c5_3_history_sync_gate` only if the connection check returns `READY_FOR_HISTORY_GATE`.
6. If history sync fails, record evidence and update the provider matrix.
7. If history sync passes, run `python -m tools.st_c5_pipeline --acquire`.
8. Continue only through the frozen pipeline sequence.
9. Do not unlock replay unless the dataset is approved by governance evidence.
