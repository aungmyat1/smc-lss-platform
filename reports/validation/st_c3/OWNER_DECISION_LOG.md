...
field below). **Remaining 4 rows genuinely `PENDING`: R-03
(`sessions.low_liquidity_filters`, untouched), R-04, R-06 (both need a
`tools/existence_check.py` research pass), and R-18
(`existence_check_floor` — now unblocked since R-02/R-16/R-20 are all
decided, but still needs an actual data run, not an owner pick).**
...
| R-04 | `wick_ratio_min` | `0.5-0.7` range | RESEARCHED, awaiting owner value | — | 2026-07-26 | Distribution scan against GBPUSD M15 complete — see `reports/validation/st_c3/R04_R06_RESEARCH_REPORT.md`. Median pierce+reclaim wick ratio is 0.33; at 0.5/0.6/0.7 only 28.5%/17.5%/9.1% of candidates pass. Owner still needs to pick the actual threshold. |
...
| R-06 | `max_sweep_age_bars` | `20-60` bars | RESEARCHED, awaiting owner value | — | 2026-07-26 | Distribution scan against GBPUSD M15 complete — see `reports/validation/st_c3/R04_R06_RESEARCH_REPORT.md`. Observed max age in data is 26 bars; the proposed 20-60 range is non-binding above ~30 (100% of candidates already pass). A real constraint would need ~10-15 bars (89.6% pass at 10). Owner still needs to pick the actual value and confirm intent (binding filter vs. rare-edge-case safety cap). |
