...
| R-22 | `instrument.selection_logic` (NEW — not previously tracked; added 2026-07-25) | Not proposed | **STILL PENDING — see mislabel note** | — | — | A 2026-07-26 submission labeled this decision "R-22" but actually supplied a *revision to R-02* (`instruments` narrowed from `[EURUSD, GBPUSD, XAUUSD]` to `[EURUSD, GBPUSD]`, rationale: XAUUSD's pip-value x SL-distance geometry exceeds the risk envelope under fixed-lot sizing at $1000 capital), not a cross-instrument tie-breaking rule. **Why this needed a scope check before recording** (same pattern as the dual-timeframe "R-03" and session-close "R-20" mismatches below): R-22 is specifically "which pair under which conditions" when multiple instruments signal concurrently near the position cap (R-12 max_positions=2, R-25 max_positions_per_instrument=1) — the submission supplied no such rule. Recorded as a **revision to R-02** instead (see that row and `reports/governance/st_c3/RCR_ST-C3_v1.0.3_REPORT.md`); R-22 itself remains genuinely unresolved. Instrument scope is now EURUSD/GBPUSD only (2 symbols), which narrows but does not eliminate the eventual need for a tie-breaking rule when both signal concurrently. |
