...
| R-31 | `liquidity_sweep_stage.sweep_reclaim_max_bars` (N_SWEEP) **(NEW — added 2026-07-26, found during `R18_EVIDENCE_BUILDER_DESIGN.md` Tier 3 gap analysis: a placeholder string, not a number, and not previously tracked under any R-item)** | Spec carried literal `"PROVISIONAL_1_TO_3"` — no number a builder could compare against | Owner decision required | **DECIDED 2026-07-27 — 2 bars, reconfirmed fresh with clean provenance (original 2026-07-26 entry's provenance was unverifiable, see `reports/governance/v1.0.6_RECONCILIATION_AUDIT.md`); see `OWNER_DECISION_LOG.md`. Folded into `specs/st-c3_v1.0.7.yaml`, NOT the quarantined v1.0.6.** | Medium — gates `SweepReclaimEvidence.reclaimed`, blocks S3 | Resolved |
| R-32 | `entry_window_stage.entry_window_bars` (MAX_ENTRY_BARS) **(NEW — added 2026-07-26, same Tier 3 gap analysis)** | Spec carried literal `"PROVISIONAL_3_TO_5_M3_BARS"` — no number a builder could compare against | Owner decision required | **DECIDED 2026-07-27 — 4 M3 bars, reconfirmed fresh with clean provenance (see R-31's note); see `OWNER_DECISION_LOG.md`. Folded into `specs/st-c3_v1.0.7.yaml`.** | Medium — gates `EntryWindowEvidence.inside_window`, blocks S11 | Resolved |
| R-33 | `sessions.london_window_utc` / `sessions.ny_window_utc` **(NEW — added 2026-07-26, same Tier 3 gap analysis)** | Spec carried literal `"PROVISIONAL_07_00_TO_10_00"` / `"PROVISIONAL_13_00_TO_16_00"` — placeholder strings, not ratified session bounds | Owner decision required | **DECIDED 2026-07-27 — ratified as final, values unchanged (London 07:00-10:00 UTC, NY 13:00-16:00 UTC), reconfirmed fresh with clean provenance (see R-31's note); see `OWNER_DECISION_LOG.md`. Folded into `specs/st-c3_v1.0.7.yaml`.** | Medium — gates `SessionWindowEvidence`, blocks S10 | Resolved |

**Note on R-18:** a quarantined line (`specs/st-c3_v1.0.6.yaml`,
`validation/st_c3/evidence_builder.py`) claimed R-18 resolved
(`signal_rate=0.0`). That claim is rejected — it used the still-provisional
OTE band as if frozen. R-18 remains open; see
`reports/validation/st_c3/R18_CLOSURE_REPORT.md` and
